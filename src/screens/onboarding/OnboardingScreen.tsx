...
const OnboardingScreen = ({ navigation, route, onComplete }: OnboardingScreenProps) => {
  const { t } = useTranslation();
  // 몇 번째 온보딩 페이지인지 route.params로 받음
  ...
  return (
    <View className="flex-1 bg-stone-100 items-center relative">
      ...
      <TouchableOpacity 
        className="absolute bottom-22 bg-sage-600 rounded-3xl border border-sage-700 active:bg-sage-700 active:scale-[0.98]"
        style={{
          left: width / 2 - 190,
          width: 381,
          height: 74,
      ...
      <TouchableOpacity 
        ...
        onPress={() => navigation.replace('SnsLogin')}
      >
