...
// temple_id로 사찰 이미지 가져오는 함수
const getTempleImage = (templeId: string) => {
...
const ReservationCard = memo<{
  ...
  const templeImage = getTempleImage(reservation.temple_id);
  ...
  return (
    <View className="bg-white rounded-2xl p-5 mb-4 border border-stone-200">
      ...
      <View className="flex-row items-start mb-4">
        {/* Temple Image */}
        <View className="w-20 h-20 rounded-xl overflow-hidden mr-4 bg-stone-100">
          {templeImage ? (
            <Image 
              source={templeImage} 
      ...
      <View className="space-y-3 mb-5">
        ...
        {reservation.participants && (
          <View className="flex-row items-center">
            <Ionicons name="people-outline" size={18} color="#4A5D23" />
            <Text className="text-base text-neutral-700 ml-3 font-medium">
              Adults: {reservation.participants.adults || 0}, 
              Teenagers: {reservation.participants.teenagers || 0}, 
              Children: {reservation.participants.children || 0}
            </Text>
          </View>
        )}
        ...
        {reservation.total_amount && (
          <View className="flex-row items-center">
            <Ionicons name="card-outline" size={18} color="#4A5D23" />
            <Text className="text-base text-neutral-700 ml-3 font-medium">
